...
class MCF_Function(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, weight, MScale):
        # size的第一个元素代表out_ch，第二个元素是in_ch
        #OutputPlane = weight.size()[0]
        # output_temp = weight.new(weight.size())
        MScale = torch.abs(MScale)
        bin = 0.02
        ...
        ctx.save_for_backward(weight, MScale)
        return weight

    @staticmethod
    def backward(ctx, gradOutput):
        weight, MScale = ctx.saved_tensors
        ...
        # pdb.set_trace()
        return gradWeight, gradMScale

class MLinear(torch.nn.Linear):
    '''
    ...
    def __init__(self, in_channels, out_channels, bias=True):
        ...
        #self.bias = bias
        #self.generate_MScale(out_channels)
        self.out_channels = out_channels
        self.MCF_Function = MCF_Function.apply
        self.bin_func = BinaryActivation()
        self.generate_MScale(out_channels)
    def generate_MScale(self, out_channels):
        self.MScale = nn.Parameter(torch.randn(out_channels, 1))
        #print(self.MScale.shape)
        #print(self.weight.shape)

    def forward(self, x):
        # inplace op
        x = self.bin_func(x)
        new_weight = self.MCF_Function(self.weight, self.MScale)
        #pdb.set_trace()
        return F.linear(x, new_weight, self.bias)

@HEADS.register_module()
class BiPointRPNHead(BaseModule):
    ...
    def _make_fc_layers(self, fc_cfg, input_channels, output_channels):
        ...
        for k in range(0, fc_cfg.__len__()):
            fc_layers.extend([
                nn.BatchNorm1d(c_in),
                MLinear(c_in, fc_cfg[k], bias=False),
