...
from mmdet3d.ops import BiPointFPModule
from ..builder import NECKS


@NECKS.register_module()
class BiPointNetFPNeck(BaseModule):
    ...
    def __init__(self, fp_channels, init_cfg=None):
        super(BiPointNetFPNeck, self).__init__(init_cfg=init_cfg)

        self.num_fp = len(fp_channels)
        self.FP_modules = nn.ModuleList()
        for cur_fp_mlps in fp_channels:
            self.FP_modules.append(BiPointFPModule(mlp_channels=cur_fp_mlps, conv_cfg=dict(type='MConv2d')))
